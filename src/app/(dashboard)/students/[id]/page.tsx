        ...
        <div className=" flex flex-col lg:flex-row h-1/5  gap-4">
          ...
          <div className="w-full lg:w-1/2 p-3 bg-sky rounded-md flex-1 flex gap-4 ">
            ...
            <div className="w-3/4 flex flex-col justify-between">
              {" "}
              {/* Added justify-between for vertical spacing */}
      ...
      <div className=" w-full xl:w-1/3 space-y-4">
        <div className=" bg-white p-4 rounded-md">
          ...
          <div className=" mt-4 gap-4 flex flex-wrap text-xs text-gray-500">
            <Link
              href={`/teachers?classId=${params.id}`}
              className=" bg-purple px-5 py-2 rounded-sm"
            >
              Student&apos;s Lessons
            </Link>
            <Link
              href={`/teachers?teacherId=${params.id}`}
              className=" bg-sky px-5 py-2 rounded-sm"
            >
              Student&apos;s Teachers
            </Link>
            <Link
              href={`/teachers?resultId=${params.id}`}
              className=" bg-skyLight px-5 py-2 rounded-sm"
            >
              Student&apos;s Results
            </Link>
            <Link href={`/teachers?examId=${params.id}`} className=" bg-yellow px-5 py-2 rounded-sm">
              Student&apos;s Exams
            </Link>
            <Link href={`/teachers?assignmentId=${params.id}`} className=" bg-pink-200 px-4 py-2 rounded-sm">
